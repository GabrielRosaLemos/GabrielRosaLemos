## 👋 Hi, I'm Gabriel Rosa

![Typing SVG](https://readme-typing-svg.herokuapp.com/?color=00F7FF&size=24&center=true&vCenter=true&width=600&lines=Python+%7C+Automation+%7C+Game+Systems;Building+real+projects;Always+learning)

🚀 **Developer focused on practical projects**  
📍Brazil | Open to opportunities

I build real-world projects involving **Python, automation and game systems**.

I enjoy creating systems that combine logic, user experience and problem solving — from voice assistants to game mechanics and educational platforms.

---

## 🌐 Where to find me

[![GitHub](https://img.shields.io/badge/GitHub-GabrielRosaLemos-181717?style=for-the-badge&logo=github)](https://github.com/GabrielRosaLemos)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Gabriel%20Rosa-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/gabriel-rosa-75050832b/)
[![Instagram](https://img.shields.io/badge/Instagram-mano__rosaa-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/mano_rosaa/)

---

## 🧠 What I do

- 🤖 Python automation
- 🎙️ Voice assistant projects
- 🎮 Game systems (missions, progression, save/load, dialogue)
- 🔗 API integrations
- 🌐 Web development (responsive websites)

---

## 🚀 Featured Projects

### 🤖 Voice Assistant in Python
Assistant with speech recognition, text-to-speech and automation features.

### 🎮 Diarcan - Gamified Educational Platform
RPG 2D project with:
- Mission system
- Level and progression system
- Save/load system
- Dialogue system
- Educational mechanics (in-game library and learning)

Concept focused on **gamification in education (EdTech)**.

---

## 🛠️ Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-FFD43B?style=for-the-badge&logo=javascript&logoColor=000)

### Web
![HTML](https://img.shields.io/badge/HTML-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### Other
- GameMaker (GML)
- Git & GitHub

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=GabrielRosaLemos&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=GabrielRosaLemos&layout=compact&theme=tokyonight)

---

## 🎯 Goal

To grow as a developer by building real projects, improving my skills and gaining professional experience.

---

⭐ Always learning  
🤝 Open to opportunities
